<?php
    ...
    $category = $conn->prepare("SELECT category FROM symptoms WHERE symptoms_name LIKE ? group by category DESC");
    $category->execute(["%".$symptom_search."%"]);
        ...
        <div class="card-body">
            <div class="form-check">
                <input class="form-check-input symptom-checkbox" type="checkbox"
                    disease="<?php echo $value['disease_id']?>" symptoms="<?php echo $value['symptoms_name']?>"
                    id="<?php echo $value['symptoms_id']?>">
