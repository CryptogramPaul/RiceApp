<?php
    ...
    $pest = $conn->prepare("SELECT * FROM pest LIMIT $page");
    $pest->execute();
...
<div class="col-xl-3 col-sm-6 col-md-4 my-1 pest-item" >
    <div class="card mb-2">
    <img src="uploads/<?php echo $value['pest_img']?>" class="card-img-top pests_img" alt="<?php echo $value['pest_img']?>" />
    <div class="card-body text-center">
        <div class="form-check">
        <label for="<?php echo $value['pest_name']?>"><?php echo $value['pest_name']?></label>
        </div>
        <div class="">
        <button class="btn btn-sm btn-outline-success" style="height: 30px;" data-bs-target="#PestModal" data-bs-toggle="modal" onclick="ViewPestInformation('<?php echo $value['id']?>')">VIEW INFORMATION</button>
        </div>
    </div>
    </div>
</div>
